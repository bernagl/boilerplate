...
Calendar.prototype.draw_day = function(day) {
  if (!this.get_day_class(day).includes('other')) {
    ...
    var day_name = createElement(
      'div',
      'day-name',
      day.locale('fr').format('ddd')
    )
    var day_number = createElement('div', 'day-number', day.format('DD'))
    var day_month = createElement(
      'div',
      'day-month',
      day.locale('fr').format('MMM')
    )
    var day_slot = createElement(
      'div',
      'day-slot',
      '',
      'data-date',
...
Calendar.prototype.select_time_slot = function(element) {
  var element_data = [
    element.parentNode.getAttribute('data-date'),
...
var time_slots = [
  {
    weekday: 1,
    slots: ['Interclasse', 'Goûter / Etude']
  },
  {
    weekday: 2,
    slots: ['Interclasse', 'TAP', 'Goûter / Etude']
  },
  {
    weekday: 3,
    slots: ['Interclasse', 'Centre de Loisirs']
  },
  {
    weekday: 4,
    slots: ['Interclasse', 'Goûter / Etude']
